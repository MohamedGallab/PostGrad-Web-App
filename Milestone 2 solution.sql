...
@ExaminerName VARCHAR (20), @fieldOfWork VARCHAR (100), @National BIT, @password varchar(20), @email varchar(50)
as
begin
	insert into PostGradUser (email,password) values(@email, @password);
	declare @id int;
	SELECT @id=SCOPE_IDENTITY();
	insert into Examiner(id, fieldOfWork, isNational, name) values (@id, @fieldOfWork, @National,  @ExaminerName);
end

Go 

create proc getIDFromMail
@email varchar(50), @Id int output
as
begin
